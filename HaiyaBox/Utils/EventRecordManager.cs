...
public class EventRecordManager
{
    ...
    public void AddRecord(string eventType, ITriggerCondParams entry)
    {
        ...
        // 限制每种事件类型的最大记录数
        while (queue.Count > MAX_RECORDS_PER_TYPE) queue.TryDequeue(out _);
        while (timedQueue.Count > MAX_RECORDS_PER_TYPE) timedQueue.TryDequeue(out _);
    }
    ...
    {
        if (!_eventRecords.TryGetValue(eventType, out var queue)) return new List<ITriggerCondParams>();
    ...
    {
        if (!_timedEventRecords.TryGetValue(eventType, out var queue)) return new List<TimedRecord>();
    ...
    public Dictionary<string, int> GetRecordCounts()
    {
        ...
        foreach (var kvp in _eventRecords) counts[kvp.Key] = kvp.Value.Count;
        return counts;
    }
    ...
    public Dictionary<string, List<ITriggerCondParams>> GetAllRecords()
    {
        ...
        foreach (var kvp in _eventRecords) result[kvp.Key] = kvp.Value.ToList();
        return result;
    }
}