...
public class GeometryTab
{
    ...
    private void DrawVisualizationSection()
    {
        ...
        {
            var enabled = DangerAreaTab.OverlayEnabled;
            if (ImGui.Checkbox("绘制图形", ref enabled)) DangerAreaTab.ToggleOverlayStatic(enabled);

            var debugPointEnabled = DangerAreaTab.DebugPointEnabled;
    ...
    private void DrawMousePositionSection()
    {
        ...
        if (ScreenToWorld(mousePos, out var wPos3D))
        {
            ...
            var distMouseCenter = GeometryUtilsXZ.DistanceXZ(wPos3D, _centerPositions[Settings.SelectedCenterIndex]);
            var angleMouseCenter = GeometryUtilsXZ.AngleXZ(_directionPositions[Settings.SelectedDirectionIndex], wPos3D,
    ...
    {
        var addDebug = Settings.AddDebugPoints;
        if (ImGui.Checkbox("添加Debug点", ref addDebug)) Settings.UpdateAddDebugPoints(addDebug);
        ImGui.SameLine();
        if (ImGui.Button("清理Debug点")) ClearDebugPoints();
    }
    ...
    {
        ImGui.Text($"Name：{enemy.Name} ({enemy.BaseId})");
        ImGui.SameLine();

        var pos = $"{enemy.Position.X:F2},{enemy.Position.Y:F2},{enemy.Position.Z:F2}";
        ImGui.Text("位置：" + pos);
        if (ImGui.IsItemHovered() && ImGui.IsMouseClicked(ImGuiMouseButton.Right)) ImGui.SetClipboardText(pos);

        ImGui.SameLine();
        var rot = $"{enemy.Rotation * 180 / float.Pi:F2}";
        ImGui.Text($"方向：{rot}");
        if (ImGui.IsItemHovered() && ImGui.IsMouseClicked(ImGuiMouseButton.Right)) ImGui.SetClipboardText(rot);
    }
    ...
    private void DrawSpellCombo(List<string> spellList)
    {
        ...
        var currentSelection = spellList[_spellListIndex] + $"#{_spellListIndex + 1}";
        if (ImGui.BeginCombo("##SpellList", currentSelection))
        {
            for (var i = 0; i < spellList.Count; i++)
            {
                var isSelected = _spellListIndex == i;
    ...
    private void DrawSpellRecord(EnemyCastSpellCondParams spellCondParams, int index)
    {
        ...
        if (ImGui.IsItemHovered() && ImGui.IsMouseClicked(ImGuiMouseButton.Right)) ImGui.SetClipboardText($"{castRot}");
    ...
    {
        var algorithmName = _calculationMode switch
        {
            0 => "旋转",
            1 => "延伸",
            2 => "面向",
    ...
    {
        for (var i = 0; i < _forwardResults.Count; i++)
        {
            ...
            {
                var result = GeometryUtilsXZ.Forward(StringToVector3(_forwardCenters[i]),
                    _forwardAngles[i] * float.Pi / 180,
                    _forwardDistances[i]);
    ...
    {
        for (var i = 0; i < _rotationResults.Count; i++)
        {
            ...
            {
                var result = GeometryUtilsXZ.RotateAroundPoint(StringToVector3(_rotationReferencePoints[i]),
                    StringToVector3(_rotationCenters[i]), _rotationAngles[i]);
    ...
    {
        for (var i = 0; i < _extensionResults.Count; i++)
        {
            ...
            {
                var result = GeometryUtilsXZ.ExtendPoint(StringToVector3(_extensionPoints[i]),
                    StringToVector3(_extensionDirections[i]), _extensionDistances[i]);
    ...
    {
        var comboLabel = _distributionMode switch
        {
            0 => "全圆均匀分布",
            1 => "直线间距分布",
            2 => "固定角度分布",
        ...
        if (_distributionMode == 1) ImGui.InputFloat("直线间距", ref _distributionSpacing, 1f, 5f, "%.2f");
    ...
    private void DrawDistributionResults()
    {
        ...
        for (var i = 0; i < _distributionPositions.Count; i++)
        {
            var pos = _distributionPositions[i];
            var line = _copyCoordinatesWithF
                ? $"({pos.X:F2}f, {pos.Y:F2}f, {pos.Z:F2}f)"
                : $"{pos.X:F2}, {pos.Y:F2}, {pos.Z:F2}";

            ImGui.Text(line);
            ImGui.SameLine();
            if (ImGui.Button("复制##" + i)) ImGui.SetClipboardText(line);
        }
    }
    ...
    {
        return p.HasValue ? $"<{p.Value.X:F2}, 0, {p.Value.Z:F2}>" : "未记录";
    }
    ...
    private void CalculateDistribution()
    {
        ...
        {
            0 => GeometryUtilsXZ.ComputeFullCirclePositions(center, _distributionRadius, _distributionFirstOffset,
                _distributionCount, _distributionClockwise),
            1 => GeometryUtilsXZ.ComputeArcPositionsByChordSpacing(center, _distributionRadius,
                _distributionFirstOffset, _distributionCount, _distributionClockwise, _distributionSpacing),
            2 => GeometryUtilsXZ.ComputePositionsByFixedAngle(center, _distributionRadius, _distributionFirstOffset,
                _distributionCount, _distributionClockwise, _fixedAngle),
            3 => GeometryUtilsXZ.ComputeArcPositionsByTotalAngle(center, _distributionRadius, _distributionFirstOffset,
                _distributionCount, _distributionClockwise, _distributionTotalAngle),
