...
public class EventRecordTab
{
    ...
    {
        // 事件记录总开关
        var eventRecordEnabled = Settings.EventRecordEnabled;
        if (ImGui.Checkbox("启用事件记录功能", ref eventRecordEnabled)) Settings.UpdateEventRecordEnabled(eventRecordEnabled);

        // 如果总开关未启用，则不继续绘制下列详细选项
        if (!eventRecordEnabled)
        {
            ImGui.TextColored(new Vector4(1, 0, 0, 1), "事件记录功能已关闭");
            return;
        }
        ...
        // 各事件类型的开关
        DrawEventTypeCheckbox("咏唱事件", "EnemyCastSpell", Settings.RecordEnemyCastSpell,
            Settings.UpdateRecordEnemyCastSpell);
        ImGui.SameLine();
        DrawEventTypeCheckbox("连线事件", "Tether", Settings.RecordTether, Settings.UpdateRecordTether);
        ImGui.SameLine();
        DrawEventTypeCheckbox("点名头标", "TargetIconEffect", Settings.RecordTargetIconEffect,
            Settings.UpdateRecordTargetIconEffect);
        ImGui.SameLine();
        ...
        // 清空按钮
        if (ImGui.Button("清空所有记录", new Vector2(120, 0))) _recordManager.ClearAllRecords();
        ImGui.SameLine();
    ...
    private void DrawEventTypeCheckbox(string label, string eventType, bool currentValue, Action<bool> updateAction)
    {
        ...
        if (ImGui.Checkbox(label, ref value)) updateAction(value);
        ImGui.SameLine();

        // 显示该事件类型的记录数量
        var records = _recordManager.GetRecords(eventType);
        var recordCount = records.Count;
        var countText = $"({recordCount}/15)";
    ...
    private void DrawEventRecords()
    {
        ...
        foreach (var eventType in enabledEventTypes) totalRecords += _recordManager.GetTimedRecords(eventType).Count;

        // 显示统计信息
        var statsText = $"共记录 {totalRecords} 条事件（显示最近15条/类型）";
        ImGui.TextColored(new Vector4(0.7f, 0.7f, 0.7f, 1), statsText);
        ImGui.Spacing();

        if (ImGui.BeginChild("EventRecordsScrollArea", new Vector2(0, 0), true))
        {
            if (enabledEventTypes.Count == 0)
                ImGui.TextColored(new Vector4(0.7f, 0.7f, 0.7f, 1), "没有开启任何事件记录");
            else
                // 为每个已开启的事件类型创建下拉列表
    ...
    private void DrawEventTypeRecords(string eventType)
    {
        ...
        // 初始化折叠状态（如果不存在）
        if (!_collapsedStates.ContainsKey(eventType)) _collapsedStates[eventType] = false; // 默认展开状态
        ...
        if (!_collapsedStates[eventType]) ImGui.SetNextItemOpen(true);
        ...
        if (ImGui.CollapsingHeader(headerText))
        {
            ...
            if (records.Count > 0)
            {
                ImGui.PushStyleColor(ImGuiCol.Button, new Vector4(0.8f, 0.3f, 0.3f, 1));
                if (ImGui.Button($"清空{typeName}记录", new Vector2(120, 0))) _recordManager.ClearRecords(eventType);
                ImGui.PopStyleColor();
                ImGui.Spacing();
            }
            ...
            {
                ImGui.Indent();
                ImGui.TextColored(new Vector4(0.5f, 0.5f, 0.5f, 1), "暂无记录");
                ImGui.Unindent();
            }
            else
            {
    ...
    private void DrawSingleRecord(TimedRecord timedRecord, int index, Vector4 color)
    {
        ImGui.Indent();
    ...
    private void DrawEnemyCastSpellRecord(EnemyCastSpellCondParams spell)
    {
        ...
        // 目标信息
        ImGui.Spacing();
        ImGui.PushStyleColor(ImGuiCol.Text, new Vector4(0.4f, 1f, 1f, 1));
    ...
    private void DrawTetherRecord(TetherCondParams tether)
    {
        ...
        // 连线信息（紫色主题）
        ImGui.PushStyleColor(ImGuiCol.Text, new Vector4(1, 0.6f, 1f, 1));
        ...
        // 连接对象
        ImGui.Spacing();
        ImGui.PushStyleColor(ImGuiCol.Text, new Vector4(0.6f, 1f, 0.6f, 1));
    ...
    private void DrawTargetIconEffectRecord(TargetIconEffectTestCondParams iconEffect)
    {
        ...
        // 被点名者信息
        ImGui.Spacing();
        ImGui.PushStyleColor(ImGuiCol.Text, new Vector4(1f, 0.8f, 0.4f, 1));
        ImGui.BulletText("被点名者");
        ImGui.PopStyleColor();

        var targetName = iconEffect.Target.Name;
        var targetId = iconEffect.Target.BaseId;
    ...
    private void DrawUnitCreateRecord(UnitCreateCondParams unitCreate)
    {
        ...
        // 单位信息（绿色主题）
        ImGui.PushStyleColor(ImGuiCol.Text, new Vector4(0.4f, 1f, 0.6f, 1));
        ImGui.BulletText("单位信息");
        ImGui.PopStyleColor();

        var name = unitCreate.BattleChara.Name;
        var dataId = unitCreate.BattleChara.BaseId;
    ...
    private void DrawGenericRecord(ITriggerCondParams record)
    {
        ...
        try
        {
            ...
            foreach (var prop in properties)
                try
                {
                    ...
                    if (value != null)
                    {
                        ...
                        if (displayedCount < 8)
                        {
                            ImGui.TextColored(new Vector4(0.8f, 0.8f, 0.8f, 1), propText);
                            displayedCount++;
                        }
                    }
                }
            ...
            // 如果有更多属性未显示，显示省略号
            if (displayedCount >= 8) ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1), "...");
        }
        catch (Exception)
        {
            ImGui.TextColored(new Vector4(1, 0, 0, 1), $"解析记录时出错");
        }
    ...
    private string FormatPropertyValue(object value)
    {
        ...
        // 处理日期时间
        if (value is DateTime dateTime) return dateTime.ToString("HH:mm:ss.fff");
    ...
    /// 获取事件类型对应的显示颜色
    /// </summary>
    private Vector4 GetEventTypeColor(string eventType)
    {
        return eventType switch
        {
            "EnemyCastSpell" => new Vector4(1, 0.5f, 0, 1), // 橙色
            "Tether" => new Vector4(1, 0, 1, 1), // 紫色
            "TargetIconEffect" => new Vector4(1, 0, 0, 1), // 红色
            "UnitCreate" => new Vector4(0, 1, 0, 1), // 绿色
            _ => Vector4.UnitW
        };
    }
    ...
    private void OnCondParamsCreateEvent(ITriggerCondParams condParams)
    {
        ...
        if (condParams is TetherCondParams tether && Settings.RecordTether) RecordEvent(tether, "Tether");
