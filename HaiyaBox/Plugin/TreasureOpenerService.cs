...
internal unsafe class TreasureOpenerService : IDisposable
{
    ...
    public bool TryInitialize()
    {
        ...
        if (!Svc.SigScanner.TryScanText("C7 44 24 ?? ?? ?? ?? ?? 48 8D 54 24 ?? 48 C7 44 24 ?? ?? ?? ?? ?? 0F 11 44 24",
                out var opcodePtr))
        {
        ...
        if (!Svc.SigScanner.TryScanText("E8 ?? ?? ?? ?? 48 8B D6 48 8B CF E8 ?? ?? ?? ?? 48 8B 8C 24",
                out var sendPacketPtr))
        {
        ...
        if (!Svc.SigScanner.TryScanText(
                "48 89 5C 24 ?? 57 48 81 EC ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 84 24 ?? ?? ?? ?? 48 8B 3D",
                out var openTreasurePtr))
        {
    ...
    private IGameObject? FindAvailableTreasure(Vector3 playerPos)
    {
        ...
        foreach (var obj in Svc.Objects)
        {
            ...
            var gameObjectPtr = (GameObject*)obj.Address;
            var treasurePos = ToNumerics(gameObjectPtr->Position);
            var distance = Vector3.Distance(playerPos, treasurePos);
    ...
    private void PruneOpenedTreasureIds()
    {
        ...
        var alive = Svc.Objects.Select(o => o.GameObjectId).ToHashSet();
    ...
    private struct UpdatePositionInstancePacket
    {
        ...
        [FieldOffset(8)] public ushort Size;

        [FieldOffset(32)] public float Rotation;

        [FieldOffset(36)] public float RotationNew;

        [FieldOffset(44)] public GameVector3 Position;

        [FieldOffset(56)] public GameVector3 PositionNew;
    }
}