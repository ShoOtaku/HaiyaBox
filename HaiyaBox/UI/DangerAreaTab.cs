...
public class DangerAreaTab
{
    ...
    private void DrawOverlaySection()
    {
        if (ImGui.CollapsingHeader("绘制开关", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            if (ImGui.Checkbox("在战斗画面中绘制 AOEShape", ref enabled)) ToggleOverlay(enabled);
    ...
    {
        if (!ImGui.CollapsingHeader("SafeZone 自动绘制", ImGuiTreeNodeFlags.DefaultOpen)) return;
    ...
    {
        if (!ImGui.CollapsingHeader("场地参数设置", ImGuiTreeNodeFlags.DefaultOpen)) return;
    ...
    {
        if (!ImGui.CollapsingHeader("安全点计算参数设置", ImGuiTreeNodeFlags.DefaultOpen)) return;
        ...
        if (_useReferencePoint)
        {
            ...
            if (_limitByMaxDistance) ImGui.InputFloat("参考点最大距离##安全点", ref _maxDistanceFromReference, 0.5f, 1f);
        }
        ...
        if (ImGui.Button("计算安全点##安全点")) ComputeSafePoints();
    ...
    {
        if (!ImGui.CollapsingHeader("安全点结果##安全点", ImGuiTreeNodeFlags.DefaultOpen)) return;
        ...
        for (var i = 0; i < _safePoints.Count; i++)
        {
    ...
    {
        if (!ImGui.CollapsingHeader("AOE 形状参数设置##aoeshape", ImGuiTreeNodeFlags.DefaultOpen)) return;
        ...
        if (ImGui.Button("添加 AOEShape##aoeshape")) AddShape();

        ImGui.SameLine();
        if (ImGui.Button("清除输入错误##aoeshape")) _lastError = string.Empty;

        if (!string.IsNullOrWhiteSpace(_lastError)) ImGui.TextColored(new Vector4(1f, 0.4f, 0.4f, 1f), _lastError);
    ...
    private void DrawShapeTypeSelector()
    {
        ...
        {
            for (var i = 0; i < ShapeLabels.Length; i++)
    ...
    {
        if (!ImGui.CollapsingHeader("当前 AOEShape 数据", ImGuiTreeNodeFlags.DefaultOpen)) return;
    ...
    private void AddShape()
    {
        ...
        var shape = _shapeTypeIndex switch
        {
            ...
            3 => new AOEShapeDonutSector(_donutSectorInnerRadius, _donutSectorOuterRadius,
                _donutSectorHalfAngleDeg.Degrees(), direction, _invertForbiddenZone),
            ...
            6 => new AOEShapeTriCone(_triConeSideLength, _triConeHalfAngleDeg.Degrees(), direction,
                _invertForbiddenZone),
    ...
    private AOEShape CreateArcCapsuleShape()
    {
        ...
        return new AOEShapeArcCapsule(_arcCapsuleRadius, _arcCapsuleAngularLengthDeg.Degrees(),
            WPos.FromVec3(orbitCenter), _invertForbiddenZone);
    }
    ...
    private void ComputeSafePoints()
    {
        ...
        if (_arenaEnabled) _arenaCenter = arenaCenter;

        if (_useReferencePoint) _referencePoint = referencePoint;
        ...
        try
        {
            ...
            if (_arenaEnabled)
            {
                ...
                else
                {
                    ...
                    _safeZoneCalculator.SetArenaBounds(new RectArenaBounds(arenaWPos, direction, _arenaHalfWidth,
                        _arenaHalfLength));
                }
            }

            foreach (var zone in BuildForbiddenZones()) _safeZoneCalculator.AddForbiddenZone(zone);
            ...
            if (_useReferencePoint)
            {
                ...
                if (_orderByReference) query = query.OrderByDistanceTo(reference);
            }
    ...
    private void SyncOverlayIfNeeded()
    {
        ...
        foreach (var entry in _entries)
        {
            ...
            var objects =
                AOEShapeDebug.BuildDisplayObjectsFor(entry.Shape, WPos.FromVec3(origin), origin.Y, entry.Color);
            if (objects != null) payload.AddRange(objects);
        }
        ...
        if (_arenaEnabled)
        {
            ...
            if (_arenaTypeIndex == 0)
            {
                payload.Add(new DisplayObjectCircle(arenaCenter, _arenaRadius, ArenaBoundsColor, ArenaOutlineThickness,
                    false));
            }
            else
            {
        ...
        if (_useReferencePoint)
        {
            ...
            payload.Add(new DisplayObjectText(referencePos + new Vector3(0f, _safePointLabelHeight, 0f),
                ReferencePointLabel, SafePointLabelBackgroundColor, SafePointLabelTextColor, 1f));
        }
        ...
        if (_showSafePoints && _safePoints.Count > 0)
        {
            ...
            for (var i = 0; i < _safePoints.Count; i++)
            {
                ...
                if (_showSafePointLabels)
                {
                    ...
                    payload.Add(new DisplayObjectText(labelPos, (i + 1).ToString(), SafePointLabelBackgroundColor,
                        SafePointLabelTextColor, 1f));
                }
            }
        }
