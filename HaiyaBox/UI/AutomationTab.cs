...
public class AutomationTab
{
    ...
    public AutomationTab()
    {
        _dutyUpdateActions = new Dictionary<DutyType, Action>
        {
            ...
            { DutyType.Sphene, () => UpdateDuty(DutyType.Sphene, ref _spheneCompletedCount, 2, "女王") },
            {
                ...
                () => UpdateDuty(DutyType.Recollection, ref _recollectionCompletedCount, 1, "泽莲尼娅")
            },
    ...
    {
        // 取出当前全局累计值
        var globalBefore = GetGlobalCount(duty);
        localCount += increment;
        var globalNew = globalBefore + increment;
        // 计算全局累计值更新配置
    ...
    private void OnDutyCompleted(IDutyStateEventArgs args)
    {
        ...
        if (_dutyUpdateActions.TryGetValue((DutyType)args.EventHandlerId, out var updateAction)) updateAction();
    ...
    public void Draw()
    {
        ...
        // 设置自动倒计时是否启用
        var countdownEnabled = Settings.AutoCountdownEnabled;
        if (ImGui.Checkbox("进本自动倒计时", ref countdownEnabled)) Settings.UpdateAutoCountdownEnabled(countdownEnabled);

        ImGui.SameLine();

        // 输入倒计时延迟时间（秒）
        ImGui.SetNextItemWidth(80f * scale);
        var countdownDelay = Settings.AutoCountdownDelay;
        if (ImGui.InputInt("##CountdownDelay", ref countdownDelay)) Settings.UpdateAutoCountdownDelay(countdownDelay);

        ImGui.SameLine();
        ImGui.Text("秒");

        // 设置自动退本是否启用
        var leaveEnabled = Settings.AutoLeaveEnabled;
        if (ImGui.Checkbox("副本结束后自动退本", ref leaveEnabled)) Settings.UpdateAutoLeaveEnabled(leaveEnabled);

        ImGui.SameLine();

        // 输入退本延迟时间（秒）
        ImGui.SetNextItemWidth(80f * scale);
        var leaveDelay = Settings.AutoLeaveDelay;
        if (ImGui.InputInt("##LeaveDutyDelay", ref leaveDelay)) Settings.UpdateAutoLeaveDelay(leaveDelay);

        ImGui.SameLine();
        ImGui.Text("秒");

        //设置是否等待R点完成后再退本
        var waitRCompleted = Settings.AutoLeaveAfterLootEnabled;
        if (ImGui.Checkbox("等待R点完成后再退本", ref waitRCompleted)) Settings.UpdateAutoLeaveAfterLootEnabled(waitRCompleted);
        ...
        ImGui.SameLine();
        if (ImGui.Button("全队AI ON")) RemoteControl.Cmd("", "/bmrai on");
        ImGui.SameLine();
        if (ImGui.Button("全队AI Off")) RemoteControl.Cmd("", "/bmrai off");
        // 修改为下拉菜单选择目标
        if (ImGui.BeginCombo("##KillAllCombo", _selectedKillTarget))
        {
            ...
            foreach (var info in partyInfo)
            {
                ...
                var isSelected = _killTargetType == KillTargetType.SinglePlayer &&
        ...
        // 添加执行按钮
        if (ImGui.Button("关闭所选目标游戏")) ExecuteSelectedKillAction();
        ...
        ImGui.SameLine();
        if (ImGui.Checkbox("自定义", ref _customRoleEnabled)) PersistRoleSelection();
        ...
        if (_customRoleEnabled)
        {
            ...
            if (!string.IsNullOrWhiteSpace(_customRoleInput)) selected.Add(_customRoleInput.Trim());
        ...
        ImGui.Text("自动排本设置:");
        // 设置自动排本是否启用
        var autoQueue = Settings.AutoQueueEnabled;
        if (ImGui.Checkbox("自动排本", ref autoQueue)) Settings.UpdateAutoQueueEnabled(autoQueue);
        ...
        ImGui.SetNextItemWidth(80f * scale);
        ImGui.SameLine();
        var queueDelay = Settings.AutoQueueDelay;
        ...
        var unrest = Settings.UnrestEnabled;
        if (ImGui.Checkbox("解限", ref unrest)) Settings.UpdateUnrestEnabled(unrest);

        //通过副本指定次数后停止自动排本 & 关游戏/关机
        {
            // 读取指定次数
            var runtimeEnabled = Settings.RunTimeEnabled;
            ...
            // 输入指定次数
            ImGui.SetNextItemWidth(80f * scale);
            var runtime = Settings.RunTimeLimit;
            ...
            if (runtimeEnabled)
            {
                ...
                ImGui.SameLine();
                if (ImGui.Checkbox("关游戏##次数", ref killGame)) Settings.UpdateKillGameAfterRunLimit(killGame);
            }
        }
        ...
        {
            var firstGroup = true;
            foreach (var category in Enum.GetValues<DutyCategory>())
            {
                // 按分组筛选副本
                ...
                foreach (var duty in duties.Where(duty =>
                             ImGui.Selectable(duty.Name, settings.SelectedDutyName == duty.Name)))
        ...
        var finalDuty = Settings.SelectedDutyName == "自定义" && !string.IsNullOrEmpty(Settings.CustomDutyName)
        ...
        if (ImGui.CollapsingHeader("自动化Debug"))
        {
            ...
            */
            ...
            if (isCrossRealmParty)
            {
                ...
                for (var i = 0; i < partyStatus.Count; i++)
                {
    ...
    private void DrawAutoUpdateSection()
    {
        ...
        var enabled = updateSettings.Enabled;
        if (ImGui.Checkbox("启用自动更新", ref enabled)) updateSettings.UpdateEnabled(enabled);

        ImGui.SameLine();
        var includePrerelease = updateSettings.IncludePrerelease;
        if (ImGui.Checkbox("包含预发布", ref includePrerelease)) updateSettings.UpdateIncludePrerelease(includePrerelease);

        ImGui.SameLine();
        if (ImGui.Button(updater.IsRunning ? "检查中..." : "立即检查")) updater.CheckNow();

        ImGui.SetNextItemWidth(80f * scale);
        var interval = updateSettings.CheckIntervalMinutes;
        if (ImGui.InputInt("检查间隔(分钟)", ref interval)) updateSettings.UpdateCheckIntervalMinutes(interval);
    ...
    private async Task UpdateAutoQueue()
    {
        ...
        {
            // 根据选择的副本名称构造实际发送命令
            var dutyName = Settings.SelectedDutyName == "自定义" && !string.IsNullOrEmpty(Settings.CustomDutyName)
                ? Settings.CustomDutyName
                : Settings.SelectedDutyName;
            if (Settings.UnrestEnabled)
                dutyName += " unrest";
            if (Settings.FinalSendDutyName != dutyName) Settings.UpdateFinalSendDutyName(dutyName);
            ...
            if (Settings.RunTimeEnabled && _runtimes >= Settings.RunTimeLimit)
            {
                ...
                // 关游戏
                if (killGame) RemoteControl.Cmd("", "/xlkill");
            }
    ...
    public static unsafe List<(string Name, bool IsOnline, bool IsInDuty)> GetCrossRealmPartyStatus()
    {
        ...
        foreach (var group in groups)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                var isOnline = data.State.HasFlag(Online);
                var isInDuty = data.State.HasFlag(InDuty);
    ...
    private static unsafe bool IsInsideCriticalEncounter(Vector3 pos, bool includeRegister = false, float radius = 20f)
    {
        ...
        foreach (ref readonly var events in instance->DynamicEventContainer.Events)
        {
            if (events.DynamicEventId == 0)
                continue;
            if (!(events.State is DynamicEventState.Battle or DynamicEventState.Warmup ||
                  (includeRegister && events.State is DynamicEventState.Register)))
                continue;
