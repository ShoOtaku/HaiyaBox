...
public class AutoSelectAddonService
{
    ...
    public void Update()
    {
        ...
        foreach (var entry in entries.Where(e => e.Enabled)) TryAutoSelect(entry);
    }
    ...
    private void TryAutoSelect(AutoSelectEntry entry)
    {
        ...
        switch (entry.SelectMode)
        {
            case AutoSelectMode.Index:
                if (entry.SelectIndex >= 0 && entry.SelectIndex < addonEntries.Count) targetIndex = entry.SelectIndex;
                break;

            case AutoSelectMode.TextMatch:
                for (var i = 0; i < addonEntries.Count; i++)
                    if (addonEntries[i].Text.Contains(entry.MatchText, StringComparison.OrdinalIgnoreCase))
                    {
            ...
            unsafe
            {
                ...
                switch (addonName)
                {
                    ...
                    case "SelectIconString":
                        var selectString =
                            new AddonMaster.SelectString(atk);
                    ...
                    case "SelectYesNo":
                        var selectYesNo =
                            new AddonMaster.SelectYesno(atk);
                        if (index == 0)
                        {
                            selectYesNo.Yes();
                    ...
                    case "Talk":
                        var talk = new AddonMaster.Talk(atk);
                        talk.Click();
    ...
    public void DrawConfigTab()
    {
        ...
        if (ImGui.Checkbox("启用自动选择功能", ref enabled)) IsEnabled = enabled;
                ...
                case AutoSelectMode.RegexMatch:
                    ...
                    if (entry.SelectMode == AutoSelectMode.RegexMatch)
                    {
                        ...
                        if (entry.UseRegex && entry.MatchRegex == null && !string.IsNullOrEmpty(entry.MatchText))
                            ImGui.TextColored(new Vector4(1, 0, 0, 1), "正则表达式无效");
                    }
