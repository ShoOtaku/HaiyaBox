...
public class 四运数据 : IMechanismState
{
    ...
    public int 南北安全;
    public int 南北钢铁;
    public List<四运玩家> 玩家分身 = new();
    public List<四运Boss分身> Boss分身 = new();
    public List<小世界塔> 四运塔 = new();
    public List<string> 左组远引导 = new();
    public List<string> 左组近引导 = new();
    public List<string> 右组远引导 = new();
    public List<string> 右组近引导 = new();
...
public class 四运玩家
{
    ...
    public uint 玩家Id;
    public Vector3 机制处理位置 = new(100, 0, 100);
    ...
    public void Update(TetherCondParams tetherCondParams)
    {
        ...
        {
            Svc.Log.Error(
                $"四运玩家.Update 错误: Left={tetherCondParams.Left?.Name?.TextValue}, Right={tetherCondParams.Right?.Name?.TextValue}, Args0={tetherCondParams.Args0}, 异常: {ex.Message}\n{ex.StackTrace}");
            throw;
        }
    }
}
...
public class 四运Boss分身
{
    ...
    public int 方位;
    public 类型 分身类型 = new();
    public Vector3 分身位置 = new(100, 0, 100);
    public int 释放轮次;
    ...
    public void Update(TetherCondParams tetherCondParams)
    {
        ...
        {
            Svc.Log.Error(
                $"四运Boss分身.Update 错误: Left={tetherCondParams.Left?.Name?.TextValue}, Right={tetherCondParams.Right?.Name?.TextValue}, Args0={tetherCondParams.Args0}, 异常: {ex.Message}\n{ex.StackTrace}");
            throw;
        }
    }
}
...
public class 小世界塔
{
    ...
    public int 分组 = 0;
    public int 方位;
    public Vector3 位置 = new(100, 0, 100);
    public Vector3 踩塔位置 = new(100, 0, 100);
    public string 分配玩家 = "";
    ...
    public void Update(IGameObject battleChara)
    {
        ...
        分组 = 位置.X > 100 ? 2 : 1;
        方位 = GeometryUtilsXZ.PositionTo4Dir(battleChara.Position,
            分组 is 1 ? new Vector3(86, 0, 100) : new Vector3(114, 0, 100));
        if (类型 is 塔类型.土 or 塔类型.火) 踩塔位置 = battleChara.Position;
        ...
        if (类型 is 塔类型.风) 踩塔位置 = battleChara.Position.WithX(battleChara.Position.X + (分组 is 1 ? 2 : -2));
